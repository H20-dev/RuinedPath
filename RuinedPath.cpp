...
void initClue() {
	const vector<string> clName = {
		...
		"19-无尽创世", "20-稳态调节", "21-域界平衡", "22-记忆共生"
	};
	...
	while (true) {
		...
		switch (choice) {
			...
			case 3: {
				...
				if (countUnlocked(clue) >= 2 + floor(2.5 * loop) && countUnlocked(badEnd) >= 6 + 4 * loop && !advanced) {
			...
			case 5: {
				cls();
				print("“感谢游玩！”");
				if (exitTry >= 41 && zombieKing)trueEnd[3].func();
