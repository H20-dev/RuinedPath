		...
		if (ch == 'H' || ch == 'h') {
			...
			if (key == '2') {
				...
				if (key == '0') {
					...
					print(purple + rev + "“翻源代码？太可恨了！” ——The Creator");
...
namespace trueEnd_ {
	...
	void Exit() { // True End 3
		...
		if(loop >= 3) trueEnd_::ServerShutdown();
		else press();
		return;
	}
	...
	while (true) {
		...
		switch (choice) {
			...
			case 5: {
				cls();
				print("“感谢游玩！”");
				if (exitTry >= 30 && zombieKing)trueEnd_::Exit();
